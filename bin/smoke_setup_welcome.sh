...
from bot.smoke.setup_welcome_probes import run_setup_welcome_smoke_flow
...
async def main() -> None:
    ...
    async def on_ready() -> None:
        try:
            ...
            try:
                ...
                print("OK: setup/welcome smoke passed")
                print(f"  network_key={result.network_key}")
                print(f"  incumbent={result.incumbent_server_name}")
                print(f"  joiner={result.joiner_server_name}")
            finally:
                await db.close()
    ...
    try:
        ...
        client_task.cancel()
        raise SystemExit(
            "FAIL: timed out waiting for setup/welcome smoke (stop the running bot and retry)"
        ) from exc
    await client_task
    if failure:
