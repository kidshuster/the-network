...
class ServerRequestService:
    ...
    async def approve_request(
        ...
        *,
        ...
        await self._context.refresh_projections()
        if result.client_role is not None:
            from bot.features.recipes.hub.leaders import grant_leaders_channel_access
    ...
    async def _provision_client_from_request(
        ...
        try:
            ...
            if isinstance(cause, discord.HTTPException):
                return _ProvisionOutcome(success=False, error=f"Discord API error: {cause}")
            raise
        except ProfileValidationError as exc:
