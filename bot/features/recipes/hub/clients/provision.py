...
async def provision_client(
    ...
    *,
    ...
    hub_names = hub_category_names()
    trailing_count = sum(
        1 for category in guild.categories if category.name.casefold() not in hub_names
    )
    category_position = next_trailing_position(
        leading_count=hub_count,
    ...
    if not isinstance(category, discord.CategoryChannel):
        detail = "; ".join(batch.failures) or "unknown layout failure"
        raise ProfileValidationError(f"Could not create client category. {detail}")
    if not isinstance(profile, discord.TextChannel):
        detail = "; ".join(batch.failures) or "unknown layout failure"
        raise ProfileValidationError(f"Could not create client profile channel. {detail}")
