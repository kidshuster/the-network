...
def build_moderator_category_overwrite(
    *,
    ...
    if allow_slash_commands:
        overwrite.use_application_commands = True  # type: ignore[misc]
    return overwrite
...
def prepare_category_create_overwrites(
    bot_member: discord.Member,
    overwrites: Mapping[
        discord.Role | discord.Member | discord.Object,
        discord.PermissionOverwrite,
    ...
    """Drop bot member and top operator role — Discord 50013 on category create."""
    stripped = strip_bot_member_overwrites(bot_member, overwrites)
    return {
        target: overwrite
        for target, overwrite in stripped.items()
        if not (
            isinstance(target, discord.Role) and target.id == bot_member.top_role.id
        )
    }
...
def _build_hub_access_overwrites(
    guild: discord.Guild,
    bot_member: discord.Member,
    access_role: discord.Role,
    human_moderator_role: discord.Role | None,
...
def _client_view_only_overwrite(*, for_category: bool) -> discord.PermissionOverwrite:
    lockdown = _category_post_lockdown() if for_category else _post_and_thread_lockdown()
    return discord.PermissionOverwrite(
        view_channel=True,
        read_message_history=True,
        **lockdown,
...
def _build_client_scoped_overwrites(
    guild: discord.Guild,
    bot_member: discord.Member,
    ...
    human_moderator_role: discord.Role | None,
    *,
    client_overwrite: discord.PermissionOverwrite | None = None,
    for_category: bool = False,
) -> dict[discord.Role | discord.Member | discord.Object, discord.PermissionOverwrite]:
    if for_category:
        base = _overwrite_base(
            guild,
            bot_member,
        ...
        base = {guild.default_role: build_everyone_hidden_overwrite()}
    if _can_configure_role(bot_member, client_role):
        base[client_role] = client_overwrite or _client_view_only_overwrite(
            for_category=for_category,
        )
...
def _build_client_roles_channel_overwrites(
    guild: discord.Guild,
    bot_member: discord.Member,
...
def build_leaders_channel_overwrites(
    ...
    *,
    ...
    return cast(
        OverwriteMap,
        _build_client_roles_channel_overwrites(
            guild,
            bot_member,
            client_roles,
            access_role,
            human_moderator_role,
            client_overwrite=build_client_leader_channel_overwrite(),
    ...
    *,
    ...
    return cast(
        ...
        _build_client_roles_channel_overwrites(
            ...
            access_role,
            human_moderator_role,
            client_overwrite=build_client_changelog_readonly_overwrite(),
            operator_role=operator_role,
        ),
    )
...
def build_hub_public_category_overwrites(
    ...
    *,
    ...
    return _build_hub_access_overwrites(
        guild,
        bot_member,
        access_role,
        human_moderator_role,
    ...
    *,
    ...
    for target, overwrite in overwrites.items():
        ...
        if (
            ...
            and isinstance(target, discord.Role)
            and target.id == bot_member.top_role.id
        ):
            # Operator role is the bot's top role — bulk/incremental 50013 if targeted.
            continue
        await channel.set_permissions(target, overwrite=overwrite, reason=reason)


async def apply_overwrites_with_fallback(
    ...
    **edit_kwargs: object,
    ...
    """Apply category or channel overwrites, using the channel sync fallback chain."""
    ...
    **edit_kwargs: object,
    ...
    if (
        ...
        try:
            ...
            pass
    if edit_kwargs:
        await channel.edit(reason=reason, **edit_kwargs)  # type: ignore[attr-defined]
...
def build_commands_channel_overwrites(
    ...
    if (
        ...
        staff.use_application_commands = True  # type: ignore[misc]
...
def build_client_category_overwrites(
    ...
    return cast(
        OverwriteMap,
        _build_client_scoped_overwrites(
            guild,
            bot_member,
            client_role,
            access_role,
...
def build_client_publish_channel_overwrites(
    ...
    return cast(
        ...
        _build_client_scoped_overwrites(
            ...
            access_role,
            human_moderator_role,
            client_overwrite=build_partner_feed_overwrite(),
        ),
    )
