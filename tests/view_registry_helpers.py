...
def make_test_view_registry() -> MagicMock:
    ...
    registry.register_client_profile_view.side_effect = (
        lambda _client_id, _network_keys, **kwargs: _view()
    )
    registry.register_subscribe_setup_view.side_effect = (
        lambda _subscription_id, _network_key: _view()
    )
    registry.register_subscription_moderation_view.side_effect = (
        lambda _subscription, _network, _setup_state: _view()
    )
    registry.register_client_profile_for_client.side_effect = (
        lambda _client, _network_keys: _view()
    )
    return registry
