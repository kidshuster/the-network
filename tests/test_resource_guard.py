...
async def test_cleanup_orphan_smoke_subscription_channels() -> None:
    from bot.smoke.resource_guard import cleanup_orphan_smoke_subscription_channels

    orphan_publish = MagicMock(spec=discord.TextChannel)
    orphan_publish.category = None
    orphan_publish.name = "stingers-publish"
    ...
    orphan_publish.name = "stingers-publish"
    orphan_subscribe.name = "stingers-subscribe"
    ...
    guild = MagicMock(spec=discord.Guild)
    guild.channels = [orphan_publish, orphan_subscribe, live_publish]
    ...
    assert manual == ["#stingers-subscribe (1002)"]
