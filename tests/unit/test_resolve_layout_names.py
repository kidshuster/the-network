...
from bot.channels.layout.loader import clear_layout_cache
...
def test_hub_channel_aliases_include_yaml_name_and_legacy() -> None:
    aliases = hub_channel_aliases(HUB_CHANNEL_MODERATOR_ONLY)
    assert hub_channel_name(HUB_CHANNEL_MODERATOR_ONLY) == aliases[0]
    assert "mod-only" in {alias.casefold() for alias in aliases}
    assert "staff-only" in {alias.casefold() for alias in aliases}
...
def test_resolve_hub_channel_matches_legacy_name() -> None:
    ...
    found = resolve_hub_channel(guild, HUB_CHANNEL_MODERATOR_ONLY)
...
def test_resolve_hub_channel_respects_category_id(
    ...
    monkeypatch.setattr(
        "bot.channels.resolve.hub_channel_aliases",
        lambda channel_id: ("ops", "commands") if channel_id == HUB_CHANNEL_COMMANDS else (),
    )
    guild = MagicMock(spec=discord.Guild)
