...
def test_join_requests_channel_name_from_layout() -> None:
    assert hub_channel_name(HUB_CHANNEL_JOIN_REQUESTS) == "join-requests"
...
def test_resolve_hub_channel_changelog_only_in_leaders_category() -> None:
    guild = MagicMock(spec=discord.Guild)
    ...
    in_leaders.name = hub_channel_name(HUB_CHANNEL_CHANGELOG)
    in_leaders.category_id = leaders_category.id
    in_leaders.is_news = MagicMock(return_value=False)

    elsewhere = MagicMock(spec=discord.TextChannel)
    elsewhere.name = hub_channel_name(HUB_CHANNEL_CHANGELOG)
    elsewhere.category_id = 99
    elsewhere.is_news = MagicMock(return_value=False)
...
def test_resolve_hub_channel_network_announcements_uses_regular_text_channel() -> None:
    guild = MagicMock(spec=discord.Guild)
    mod_category = MagicMock(spec=discord.CategoryChannel)
    mod_category.name = hub_category_name(HUB_CATEGORY_MODERATION)
    mod_category.id = 10
