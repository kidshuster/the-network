...
class PermissionProvisionProbeResult:
...
async def run_permission_provision_probe(
    guild: discord.Guild,
    bot_member: discord.Member,
    ...
    *,
...
) -> PermissionProvisionProbeResult:
    """Verify the permissions needed by operator and client-provisioning APIs."""
    ...
    return PermissionProvisionProbeResult(
        operator_steps=tuple(operator_steps),
        provision_steps=tuple(provision_steps),
    )


async def run_configured_permission_provision_probe(
    guild: discord.Guild,
    bot_member: discord.Member,
    settings: Settings,
) -> PermissionProvisionProbeResult:
    """Resolve configured roles and run the standalone destructive permission probe."""
    access_role = resolve_access_role(guild, role_name=settings.network_access_role_name)
    return await run_permission_provision_probe(
        guild,
        bot_member,
        access_role,
...
async def ensure_smoke_network_key(
    ...
    *,
    ...
    existing = await context.store.networks.get_by_key(default_key)
...
async def run_hub_rebuild_smoke_flow(
    ...
    *,
    ...
    network_definitions = [
        (network.key, network.display_name)
    ...
    try:
        ...
        recreated = {}
        for key, display_name in network_definitions:
            ...
            if not create.success or create.network is None:
                raise RuntimeError(create.error or f"network recreate failed for {key!r}")
            recreated[key] = create.network

        recreated_smoke_network = recreated.get(state.network_key)
        if recreated_smoke_network is None:
            raise RuntimeError(f"Smoke network {state.network_key!r} was not restored.")

        restored_keys = {network.key for network in await context.store.networks.list_all()}
        expected_keys = {key for key, _display_name in network_definitions}
        if restored_keys != expected_keys:
            raise RuntimeError(
                "Network definitions changed during rebuild: "
                f"expected={sorted(expected_keys)}, restored={sorted(restored_keys)}"
            )

        subscription = await context.store.clients.get_subscription(
            state.client_id,
            recreated_smoke_network.id,
        )
